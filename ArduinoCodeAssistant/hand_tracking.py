...
# C:\Users\saswt\AppData\Local\Programs\Python\Python312\python.exe -m pip install opencv-python numpy mediapipe

###################
wCam, hCam, delta_angle_threshold, speed_ratio_threshold = int(sys.argv[1]), int(sys.argv[2]), int(sys.argv[3]), float(sys.argv[4])
###################

###################
output_delta_angle = 0
    ...
    while cap.isOpened():
        ...
        if results.multi_hand_landmarks:
            ...
            if np.linalg.norm(origin_tip_vector) < np.linalg.norm(origin_base_vector):
                ...
                origin_pinkyBase_vector = np.array(landmarks[18]) - np.array(landmarks[0])
                # 새끼손가락이 펴져있는 경우
